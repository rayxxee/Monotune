...
export default function ThreadDetail({ postId, user, token, onBack, onNavigateToProfile }: { postId: number, user: any, token: string, onBack: () => void, onNavigateToProfile?: (id: string) => void }) {
  ...
  return (
    <div className="flex-1 p-4 md:p-8 lg:p-12 overflow-y-auto bg-white flex flex-col items-center">
      <div className="max-w-3xl w-full flex flex-col gap-12">
        ...
        {/* OP POST */}
        <div className="brutalist-border-thick p-6 md:p-12 flex flex-col gap-8 bg-grey-silver">
          <div className="flex items-center gap-4 border-b-4 border-black pb-4">
             <h4 
               className="font-bold text-xl tracking-tighter uppercase cursor-pointer hover:underline"
               onClick={() => onNavigateToProfile?.(data.post.user_id)}
             >{data.post.username}</h4>
             <span className="text-xs font-bold text-grey-mid uppercase">ORIGINAL SIGNAL</span>
          </div>
          <div>
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none uppercase mb-4">{data.post.title || "UNTITLED"}</h2>
            ...
            {data.comments.map((c: any) => (
              <div key={c.id} className="brutalist-border p-6 bg-white flex flex-col gap-2">
                 <div className="flex justify-between text-[10px] font-bold tracking-widest uppercase opacity-40">
                    <span 
                      className="cursor-pointer hover:opacity-100 hover:underline transition-opacity"
