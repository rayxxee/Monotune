...
export default function AdminDash({ user, token }: { user: any, token: string }) {
  ...
  return (
    <div className="flex-1 p-4 md:p-8 lg:p-12 overflow-y-auto bg-grey-silver">
      <div className="max-w-6xl mx-auto flex flex-col gap-12">
        
        <div className="flex items-center gap-4 border-b-8 border-black pb-4">
           <Shield size={64} className="text-black" />
           <h1 className="text-5xl md:text-8xl font-black tracking-tighter uppercase">ADMIN PANEL</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
           {[
             ...
             <div key={i} className="bg-white brutalist-border p-4 md:p-8 flex flex-col gap-4 shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
                <div className="flex justify-between items-center opacity-40">
                   {s.icon}
                   <span className="text-[10px] font-bold tracking-widest">{s.label}</span>
                </div>
                <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none">{s.val}</h2>
             </div>
           ))}
        </div>

        {/* MANUAL REVIEW QUEUE */}
        <div className="flex flex-col gap-6 md:gap-8 bg-white brutalist-border p-6 md:p-12">
        ...
        <div className="bg-black text-white p-4 md:p-8 flex flex-col gap-4">
